...
function daemon_loop() {
  ...
  trap 'log_info "Received SIGTERM, shutting down gracefully"; rm -f "$(get_pid_file)"; exit 0' TERM
  trap 'log_info "Received SIGINT, shutting down gracefully"; rm -f "$(get_pid_file)"; exit 0' INT
